...
class FleetRecord extends Model
{
    ...
    public function getDataTable(Request $request)
    {
        ...
        $query = FleetRecord::where("id_fleet", $request->id_fleet);
    ...
    {
        $files=Asset::where('id_reference',$element->id)->where("type","fleet_record")->get();
        $element['date_format']=Carbon::createFromFormat('Y-m-d H:i:s', $element->created_at)->format('m/d/Y');
        $element['type_name'] = $element['type']===0?'Dielectric Test':($element['type']===1?'DOT':'Custom');
        $element['files'] = $files;
